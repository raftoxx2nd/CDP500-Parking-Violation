...
FIXED_VIDEO_SOURCE = "e:\\Programs\\cdp 400 ver 0.2\\input\\Fast 1080p30-20251022 134935.mp4"
CONF_THRESHOLD = 0.3
IOU_THRESHOLD = 0.5
...
class VideoStream:
    ...
    def __init__(self, src=0):
        ...
        # Use a queue of maxsize 1 to always get the latest frame
...
def run_violation_detection():
    ...
    time.sleep(2.0) # Give the stream thread time to start and fill queue
    ...
    # Get FPS (Note: cap.get(cv2.CAP_PROP_FPS) is unreliable for streams)
    # We'll calculate it manually
    fps = 30 # Assume 30 FPS if calculation fails
    ...
    print(f"\n🚀 Starting real-time detection on {DEVICE.upper()}...")
    ...
    try:
        while True:
            ...
            results = model.track(frame, persist=True, verbose=False, device=DEVICE, 
                                  conf=CONF_THRESHOLD, iou=IOU_THRESHOLD, classes=[2, 3, 5, 7])[0] # car, motorcycle, bus, truck
            ...
            if results.boxes.id is not None:
                ...
                # Set of IDs seen in the current frame
                    ...
                    if is_in_zone and label in ['motorcycle', 'motorbike']:
                        ...
                        if idle_timers[track_id] >= violation_threshold_frames:
                            if track_id not in violation_history:
                                ...
                                # Save snapshot with a web-accessible path
